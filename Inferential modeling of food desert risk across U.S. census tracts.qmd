---
title: "Inferential modeling of food desert risk across U.S. census tracts"
author: "Inaya Ahmed and Kaodinna Onwumechili"
format: pdf
---

NOTE: Using `readxl::read_xlsx()` with the original Excel file was not working for me, so I converted the sheet with the data to a CSV file and used `read.csv()`
...
summary(data_regression$TractHUNV)
```
...
ggplot(
  std_coefs, aes(x = reorder(Predictor, abs(Coefficient)), y = Coefficient)
) +
...
data.frame(
    "Model" = c("LASSO", "Logit"),
    "Threshold" = c(best_t, best_th),
    "Sensitivity" = c(
      best_conf_mat$byClass[["Sensitivity"]], best_cm$byClass[["Sensitivity"]]
    ),
    "Balanced Accuracy" = c(
      best_conf_mat$byClass[["Balanced Accuracy"]], 
      best_cm$byClass[["Balanced Accuracy"]]
...
```{r}
plot(f_pos_rate, t_pos_rate, main = "Logit ROC Curve", col = "blue")
abline(0, 1, col = "red")
```
